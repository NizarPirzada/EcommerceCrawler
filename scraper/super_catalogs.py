...
class CDW(SeleniumBaseScraper):
    ...
    def process_brand_page(self):
        ...
        while 1:
            if row_element.has_attr('class') and self.brands_selector in " ".join(row_element.get('class')):
                break
...
class TecNec(SeleniumBaseScraper):
    ...
    def process_brand_page(self):
        ...
        # brand_main_element = soup.find_all(brand_selector_paths["main"][0], brand_selector_paths["main"][1])
        # for elem in brand_main_element:
        #     brand_rows_element = elem.find_all("ul")
        #     print("len of brands", len(brand_rows_element))
        #     for brand_row_element in brand_rows_element:
        #         a_tags = brand_row_element.find_all("a")
        #         self.fetch_brand_a_tags(a_tags)
    ...
    def process_listing_page(self):
        try:
            ...
            items_elements = soup.find_all(items_selector_paths["main"][0], items_selector_paths["main"][1])
            for item_element in items_elements:
                try:
                    ...
                    item = ProductItem(
                        itemName=utils.clean_text(item_element.select_one("h2 > a").text),
                        itemUrl=item_element.select_one("h2 > a")["href"],
                        status=None,
                        vendorSku=item_element.select_one(".cat_item_part_number > a").text,
