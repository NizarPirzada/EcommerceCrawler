...
def get_collection(collection_obj ,query_obj):
    if enums.Database.FireBase.value == settings.DB:
        return firebase.get_collection(collection_obj=collection_obj, query_obj=query_obj)
    return []
...
def update_document(collection, document_uuid, json_data):
    if enums.Database.FireBase.value == settings.DB:
        firebase.update_document(collection=collection, document_uuid=document_uuid, data=json_data)
