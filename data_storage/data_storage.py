...
class DataStorage:
    ...
    def save_prices(self, query_obj, prices):
        for price in prices:
            ...
            query_obj["nested_collection"]["nested_collection"]["nested_collection"]["query_obj"] = query_data
            ...
            if not price_document_id:
                query_obj["nested_collection"]["nested_collection"]["nested_collection"]["query_obj"] = None
                db.save_data(
                    query_data=query_obj, new_data=price
                )

            else:
                if price["amount"] == price_document_data["amount"]:
                    db.update_data(query_obj, price_document_id, {"lastSeen": price["lastSeen"]})
                else:
    ...
    def set_items(self, data):
        self.items = utils.convert_class_objects_to_list_dic(data)
    ...
    def save_items(self, catalog_url):
        ...
        item_default_query_obj["nested_collection"] = copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)

        item_default_query_obj["nested_collection"]["collection"] = enums.Collections.Catalogs.value
        item_default_query_obj["nested_collection"]["document_id"] = self.catalog_document_id
        item_default_query_obj["nested_collection"]["nested_collection"] = copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)

        item_default_query_obj["nested_collection"]["nested_collection"]["collection"] = enums.Collections.Items.value
        ...
        for item in self.items:
            ...
            item_query_obj["nested_collection"]["nested_collection"]["query_obj"] = query_data
            prices = item["prices"]
            del item["prices"]
            db.save_update_data(
                query_data=item_query_obj, new_updated_data=item
            )
            ...
            item_query_obj["nested_collection"]["nested_collection"]["document_id"] = item_document_id
            item_query_obj["nested_collection"]["nested_collection"]["query_obj"] = None
            item_query_obj["nested_collection"]["nested_collection"]["nested_collection"] = \
                copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)
            item_query_obj["nested_collection"]["nested_collection"]["nested_collection"]["collection"] = \
                enums.Collections.Prices.value
    ...
    def save_catalogs(self):
        ...
        catalog_default_query_obj["nested_collection"] = copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)
        catalog_default_query_obj["nested_collection"]["collection"] = enums.Collections.Catalogs.value
        ...
        for catalog in self.catalogs:
            ...
            # catalog_query_obj["nested_collection"]["query_obj"][0]["value"] = enums.Collections.Catalogs.value
            # db.save_update_data(
            #     query_data=catalog_query_obj, new_updated_data=catalog
            # )
            ...
            if not catalog_document_id:
                # query_obj["nested_collection"]["nested_collection"]["nested_collection"]["query_obj"] = None
                db.save_data(
                    query_data=catalog_query_obj, new_data=catalog
                )
    ...
    def set_catalog_id(self, catalog_url):
        ...
        query_obj["nested_collection"] = copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)
        query_obj["nested_collection"]["collection"] = enums.Collections.Catalogs.value
        query_obj["nested_collection"]["query_obj"] = copy.deepcopy(settings.DEFAULT_CATALOG_QUERY_OBJ)
        ...
        if not self.catalog_document_id:
            data = [
                ...
                {
                    "collection": enums.Collections.Catalogs.value,
                    "document_id": None
                }
            ]
    ...
    def update_actor_time(self, document_path="", field_name=""):
        ...
        actor_query_obj["collection"] = "Vendors"
        actor_query_obj["document_id"] = "9b27bf7ed43d426dbc5b2287b9718e53"
        actor_query_obj["nested_collection"] = copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)
        actor_query_obj["nested_collection"]["collection"] = "Catalogs"
        actor_query_obj["nested_collection"]["document_id"] = "58973f23d33d4a77ba1bf70769c63996"
        actor_query_obj["nested_collection"]["nested_collection"] = copy.deepcopy(settings.DEFAULT_FIRESTORE_QUERY_OBJ)
        actor_query_obj["nested_collection"]["nested_collection"]["collection"] = "Tasks"
        # actor_query_obj["nested_collection"]["nested_collection"]["document_id"] = "Z9BSTGuVDir5RWFzjVOz"

        actor_document_id = "Z9BSTGuVDir5RWFzjVOz"

        actor_schedule = {
            field_name: datetime.utcnow()
        }
        db.update_data(actor_query_obj, actor_document_id, actor_schedule)
        # db.update_document("Tasks", "Z9BSTGuVDir5RWFzjVOz", {"actorEndTime": datetime.utcnow()})
