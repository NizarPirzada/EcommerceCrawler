        ...
        while True:
            ...
            if query_obj.get("query_obj"):
                for item in query_obj.get("query_obj"):
                    query_ref = query_ref.where(item["field"], item["operator"], item["value"])
            query_obj = query_obj.get("nested_collection")
            if not query_obj:
                break
...
def get_collection(collection_obj=None, query_obj={}):
    db = get_db_client()
    if collection_obj:
        collections = collection_obj.collection(query_obj["collection"]).document(query_obj["document_id"]).collections()
    else:
        collections = db.collection(query_obj["collection"]).document(query_obj["document_id"]).collections()
...
def update_document(collection=None, document_uuid=None, data={}):
    db = get_db_client()
    # doc_ref = db.collection(collection).document(document_uuid)
    #
    # # Set the capital field
    # doc_ref.update(data)
    city_ref = db.collection(u'/Vendors/9b27bf7ed43d426dbc5b2287b9718e53/Catalogs/58973f23d33d4a77ba1bf70769c63996/Tasks').document(u'Z9BSTGuVDir5RWFzjVOz')
    city_ref = db.collection(u'Vendors').document(u'9b27bf7ed43d426dbc5b2287b9718e53').collection(u'Catalogs').document(u'58973f23d33d4a77ba1bf70769c63996').collection(u'Tasks').document(u'Z9BSTGuVDir5RWFzjVOz')
    city_ref.update({
        u'actorEndTime': datetime.utcnow()
    })
...
def main():
    data = [
        {
            "collection": "Vendors",
            "document_id": "zzzzzzzzzzzzzzzzzzzy"
        },
        {
            "collection": "T",
            "document_id": None
    ...
    update_document("Tasks", "Z9BSTGuVDir5RWFzjVOz", {"actorEndTime": datetime.utcnow()})
